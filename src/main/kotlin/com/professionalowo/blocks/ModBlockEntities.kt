...
object ModBlockEntities : Initializer() {
    ...
    val ALTAR_CORE_BLOCK_ENTITY: BlockEntityType<AltarCoreBlockEntity> =
        register("altar_core", ModBlocks.ALTAR_CORE_BLOCK) { pos, state -> AltarCoreBlockEntity(pos, state) }

    val ALTAR_PEDESTAL_BLOCK_ENTITY: BlockEntityType<AltarPedestalBlockEntity> =
        register("altar_pedestal", ModBlocks.ALTAR_PEDESTAL_BLOCK) { pos, state ->
            AltarPedestalBlockEntity(
                pos,
                state
            )
        }
